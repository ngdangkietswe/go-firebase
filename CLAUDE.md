...
*Tip:* Most commands can be run from the repository root. Use `make` targets if a `Makefile` is present (e.g.,
...
2. **Request Handling**
    ...
    - Services interact with the **data layer** (`internal/data/ent` for relational data, `internal/firebase` for push
      notifications).
...
5. **Firebase Integration**
    - The Firebase client (`internal/firebase`) wraps the Firebase Admin SDK for tasks such as sending FCM messages and
